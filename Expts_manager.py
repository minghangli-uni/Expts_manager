...
class Expts_manager(object):
    ...
    def setup_ctrl_expt(self,yamlfile):
        ...
        self.template_branch_name = self.expt_yaml["template_branch_name"]
        self.base_branch_name = self.expt_yaml["base_branch_name"]
        base_rel_path = self.template_branch_name
        self.test_path = self.expt_yaml["test_path"]

        # detect parameter changes in `expt_mom.yaml`
        ...
        self.base_path = os.path.join(self.test_path,base_rel_path)

        # create ctrl branch from a designated repo defined in `expt_mom.yaml`
        if os.path.exists(self.base_path):
            print(f"Base path is already created located at {self.base_path}")
        else:
            print(f"template ctrl clone {self.template_branch_name} from {template} to {base_rel_path} as a branch of {self.base_branch_name}!")
            command = f"payu clone -B {self.template_branch_name} -b {self.base_branch_name} {template} {self.base_path}"
            subprocess.run(command, shell=True, check=False)
            print(f"Baseline is created located at {self.base_path}")
        ...
        # define `startfrom` following `expt_mom.yaml`
    ...
    def manage_expts(self):
        ...
        for i in range(len(self.param_dict_change_list)):
            ...
            if os.path.exists(expt_path):
                ...
                metadata["keywords"] = f"{self.template_branch_name}, {BRANCH_PERTURB}, {keywords}"
